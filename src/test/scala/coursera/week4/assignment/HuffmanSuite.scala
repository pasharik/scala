...
class HuffmanSuite {
  ...
  @Test def `combine with order`: Unit = {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 2))
    assertEquals(List(Leaf('x',2), Fork(Leaf('e',1),Leaf('t',2),List('e', 't'),3)), combine(leaflist))
  }


  @Test def `until testing`: Unit = {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 2))
    println(until(singleton, combine)(leaflist))
  }

  @Test def `createCodeTree testing`: Unit = {
    val chars = "etxe".toList
    println(createCodeTree(chars))
  }

  @Test def `decodedSecret check`: Unit = {
    println(decodedSecret)
  }

  @Test def `convert check`: Unit = {
